...
* **执行步骤与原理命令**：
1. 创建伪应用目录结构：`mkdir -p ~/.AIToBox/Apps/ChromeATB.app/Contents/MacOS`
...
USER_DATA="$HOME/.AIToBox/Data/ChromeATB"
# 直接带参数启动原版二进制，瞬间秒开
...
由于在之前的“壳劫持”设计中，我们已经将应用本体（`/Applications/WeChat_ATB.app`）与用户数据（`~/.AIToBox/WeChat_Data/`）进行了**物理级解耦**，因此分身的升级其实可以做到“丝滑且无损”。
...
1. 杀死当前运行的分身进程。
2. 直接删除旧的 `/Applications/WeChat_ATB.app`（由于数据全在 `~/.AIToBox/` 下，这个删除动作**完全不会丢失任何用户聊天记录和登录状态**）。
