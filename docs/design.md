...
系统内置一套 **App Recipes (分身规则引擎)**，根据拖入应用的 `Bundle ID` 自动匹配最优克隆模式，或允许高级用户手动干预。
...
* **一键导出导入**：用户调通了某个小众应用的“完美分身配置”后，可以生成一段类似 Dockerfile 的 yaml 文本分享到社区。工具在运行时会自动拉取 Github 上的最佳实践分身规则，实现“一键克隆，开箱即用”。
...
**ATB 解法：所见即所得的“规则编辑器” (GUI Rule Builder)**
...
* 这使得 ATBClone 具备了病毒式传播的社交属性——用户可以在论坛里直接互传 YAML 分身规则，一键解决新应用的多开难题。
