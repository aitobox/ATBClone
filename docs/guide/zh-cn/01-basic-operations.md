...
* **默认目录（`~/Applications` —— 强烈推荐）**：安装到当前用户的应用程序目录。**全程无需管理员密码，免 Root 提权。**
...
* **从访达或程序坞启动**：直接双击 `~/Applications/WeChat2.app`。
...
* **保留数据（默认不勾选）**：仅从 `~/Applications` 中移除 `.app` 应用程序包，聊天记录与数据库完整保存在 `~/ATBClone/Data/` 中。后续如需恢复，随时使用同名再次克隆即可瞬间找回所有记录。
