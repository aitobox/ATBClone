...
### 📢 サイドバーへの ATBCmder プロモーションカードおよび [AD] バッジ追加
- **エコシステム連携と UI 拡張**:
  - ナビゲーションサイドバーの中央スペースに、姉妹ツール ATBCmder（`https://aitobox.com/atbcmder`）のプロモーションカードを追加。
  - 半透明の角丸背景、明確な `[AD]` 広告表示、高解像度アプリアイコン、多言語サブタイトル、ブラウザ起動ボタンを備えた macOS ネイティブデザインを採用。
  - 全 9 言語の i18n に対応し、設定画面での言語変更時にカードのタイトル・説明文・ボタン文言が即座に動的更新。
  - ATBCmder アイコンリソースおよび安全なフォールバックを伴うリソース読み込み関数を追加。

### 🛡️ CI/CD リリースノート保持保護とパッケージング強化
- **リリース自動化の堅牢性向上**:
  - `build_cli.sh` の `dist/` クリーンアップ時に、生成済みの `dist/release_notes.md` が削除されて GitHub リリースにノートが含まれなくなる不具合を修正。
  - `scripts/build_release_packages.sh` にリリースノートの自動バックアップおよび復元処理を追加。
  - `.github/workflows/release.yml` に Git タグ注釈およびコミット履歴によるフォールバック生成処理を追加し、リリースドキュメントの欠落を防止。

### 🧪 包括的な品質保証と自動テスト
- **テストスイートの拡張**:
  - 自動テストを 588 件に拡張し合格率 100% を維持。サイドバーカードのレイアウト、外部リンク遷移、言語切替時の動的反映、パッケージングスクリプトをカバー。

---

## [v1.7.0] - 2026-09-13

### 🔄 アプリ内自動アップデート＆サイレントインストーラー
- **エンドツーエンドの自動更新システム**:
  - `UpdateService` を実装し、GitHub リリースマニフェスト `latest.json` を取得してセマンティックバージョニングによる新バージョン自動検出に対応。
  - Apple Silicon (arm64) および Intel (x86_64) アーキテクチャを自動判定して適切な DMG をストリーミングダウンロード（リアルタイム進捗パーセンテージ表示）。
  - リリースマニフェストの SHA256 ハッシュによる自動整合性検証を実施し、破損や改ざんされたファイルのインストールを防止。
  - 設定済みのプロキシ環境変数（`HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY`）を自動継承し、制限ネットワーク下でも安定した更新が可能。
  - 独立した一時ディレクトリへの DMG マウント、`ATBClone.app` の置換、macOS Gatekeeper 検疫属性の自動削除（`xattr -cr`）、安全なアンマウントを伴うサイレントインストールを実現。

### ⚙️ GUI 設定画面のアップデートフロー
- **直感的な更新ステータス管理**:
  - `SettingsView` に「アップデートを確認」ボタンおよびリアルタイムステータス表示を追加。
  - 多段階の状態遷移表示：確認中 → 新バージョンあり（バージョン番号・更新履歴表示）/ 最新状態 → ダウンロード中（進捗率表示）→ インストール中 → 完了 / 再起動の案内。
  - タイムアウト、HTTP エラー、チェックサム不一致時の分かりやすいエラー通知を実装。

### 🌐 9 言語の完全ローカライズ
- **多言語対応**:
  - サポート対象の全 9 言語において、更新ボタン、ステータスラベル、エラーダイアログの翻訳を完備。

### 📦 CI/CD リリースマニフェスト自動生成 (`latest.json`)
- **メタデータの自動化**:
  - `scripts/build_release_packages.sh` を拡張し、リリースノート抽出、DMG の SHA256 算出、および `dist/latest.json` の自動出力を実装。
  - `.github/workflows/release.yml` により、GitHub Releases へ `latest.json` を自動配信。

### 🧪 品質保証とテスト拡充
- **テストスイートの強化**:
  - 自動テストスイートを 585 件に拡充し、UpdateService、UI フロー、チェックサム検証、i18n を網羅して 100% の合格率を達成。

---

## [v1.6.0] - 2026-09-13

### 🪟 トレイ終了時のデッドロック解消とシャットダウンライフサイクルの強化
- **安全かつ確実なプロセスの終了**:
  - アプリをトレイに最小化した状態で、トレイアイコンや Dock の右クリックメニューから「終了」を選択した際にデッドロックが発生しアプリが終了しない不具合を修正。
  - `ATBCloneApp` に `_is_exiting` ガードフラグを導入し、終了処理中に `_on_window_close` がウィンドウの終了を遮断・ブロックしないよう改善。
  - `exit_application()` 実行時に `set_macos_dock_visible(True)` を不要に呼び出していた処理を削除し、アクティベーションポリシーの変更による Cocoa イベントループの再突入と終了阻害を根本から解消。
  - Dock からの終了（`_on_app_exit`）とトレイメニューからの終了（`exit_application`）の両ルートを強化し、トレイを確実に破棄して即座に終了するよう最適化。

---

## [v1.5.1] - 2026-09-13

### 💬 KakaoTalk の組み込みレシピ追加
- **ネイティブ複数インスタンスのサポート**:
  - KakaoTalk (`com.kakao.KakaoTalkMac`) の組み込みレシピを追加。Cocoa ハードクローン戦略、サンドボックス解除、および独立したデータディレクトリ (`HOME`/`TMPDIR`) をサポート。
  - 組み込みレシピライブラリを 34 以上のアプリケーションに拡充。
  - 検証範囲をドキュメントに明記（同時起動および通知機能の動作を確認済み）。

### 🚀 GitHub Actions arm64 自動リリース・パッケージングワークフロー
- **継続的デプロイとビルド自動化**:
  - バージョンタグ（`v*`）プッシュ時に自動起動する `.github/workflows/release.yml` を追加。
  - CLI スタンドアロンバイナリ (`ATBCloneCli-arm64.tar.gz`) および GUI インストーラ (`ATBClone-*.dmg`) を自動ビルドし、SHA256 チェックサムを生成。
  - リリース成果物の統合パッケージングスクリプト `scripts/build_release_packages.sh` を追加。
  - `scripts/build_cli.sh` および `scripts/build_release_packages.sh` における bash `set -u` の空配列展開エラーを修正。

### 🛡️ GUI ビルドスキャフォールディングとネットワーク耐性の強化
- **不完全ビルドの自動修復**:
  - `scripts/build_gui.sh` の整合性チェックを強化し、`Python.framework`、`ATBClone` 実行可能ファイル、および `Stub` を厳密に検証。
  - ネットワーク不安定によるダウンロード破損を検知した場合、不完全な構造を自動削除して再生成。

### 🔒 セキュリティポリシー策定 (SECURITY.md)
- **脆弱性報告ガイドライン**:
  - `.github/SECURITY.md` を追加し、サポート対象バージョン、プライベート脆弱性報告窓口、対応 SLA を明確化。

### 🧪 品質保証とテスト拡充
- **テストスイートの拡大**:
  - 自動テストスイートを 561 件に拡充し、合格率 100% を達成。
