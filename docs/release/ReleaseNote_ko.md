...
### 📢 사이드바 내 ATBCmder 프로모션 카드 및 [AD] 뱃지 추가
- **생산성 도구 연동 및 UI 강화**:
  - 탐색 사이드바의 여백 영역에 자매 생산성 도구인 ATBCmder(`https://aitobox.com/atbcmder`) 프로모션 카드를 추가했습니다.
  - 반투명 둥근 카드 배경, 명확한 `[AD]` 광고 표기, 고해상도 앱 아이콘, 다국어 부제목 및 기본 브라우저 열기 버튼을 포함한 macOS 네이티브 스타일을 적용했습니다.
  - 9개 국어 i18n 시스템을 완벽 지원하여 환경설정에서 언어를 변경할 때 카드의 제목, 설명 및 버튼 문구가 실시간으로 갱신됩니다.
  - ATBCmder 아이콘 리소스 및 안전한 대체 경로를 지원하는 리소스 헬퍼를 추가했습니다.

### 🛡️ CI/CD 릴리즈 노트 보존 및 패키징 파이프라인 강화
- **배포 자동화 안정성 개선**:
  - `build_cli.sh`에서 `dist/` 폴더를 정리할 때 사전에 생성된 `dist/release_notes.md`가 삭제되어 GitHub Releases에 배포 노트가 누락되던 문제를 해결했습니다.
  - `scripts/build_release_packages.sh`에 릴리즈 노트 자동 백업 및 복원 메커니즘을 추가했습니다.
  - `.github/workflows/release.yml`에 Git 태그 주석 및 커밋 기록 기반의 대체 생성 로직을 구축하여 배포 설명이 유실되지 않도록 보장했습니다.

### 🧪 품질 보증 및 자동화 테스트
- **테스트 케이스 확장**:
  - 자동화 테스트를 588개로 확장하여 100% 통과율을 유지하며, 사이드바 카드 레이아웃, 외부 링크 이동, 다국어 동적 갱신 및 패키징 스크립트를 검증했습니다.

---

## [v1.7.0] - 2026-09-13

### 🔄 앱 내 자동 업데이트 및 자동 설치 하위 시스템
- **엔드투엔드 자동 업데이트 지원**:
  - `UpdateService`를 구현하여 GitHub 릴리즈 매니페스트 `latest.json`을 조회하고 시맨틱 버전을 비교하여 최신 버전을 감지합니다.
  - Apple Silicon(arm64) 및 Intel(x86_64) 아키텍처를 자동으로 인식하여 적합한 DMG를 스트리밍 청크 방식으로 다운로드하며 실시간 진행률(%)을 표시합니다.
  - 다운로드된 DMG 파일의 SHA256 체크섬을 매니페스트와 대조하여 파일 손상 및 변조를 사전에 차단합니다.
  - 설정된 네트워크 프록시(`HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`)를 자동 상속하여 제한된 네트워크 환경에서도 안정적인 업데이트를 보장합니다.
  - 임시 마운트 지점을 통한 DMG 마운트, `ATBClone.app` 교체, macOS Gatekeeper 격리 속성 해제(`xattr -cr`) 및 볼륨 언마운트를 수행하는 자동 설치를 구현했습니다.

### ⚙️ GUI 설정 화면의 업데이트 흐름 통합
- **시각적 업데이트 상태 안내**:
  - `SettingsView`에 '업데이트 확인' 버튼 및 실시간 상태 안내 레이블을 추가했습니다.
  - 다단계 시각적 상태 전환: 확인 중 → 최신 버전 발견(버전 및 릴리즈 노트 표시) / 최신 버전 유지 중 → 다운로드 중(진행률 표시) → 설치 중 → 완료 / 재시작 안내.
  - 네트워크 타임아웃, HTTP 오류 및 체크섬 불일치에 대한 명확한 오류 안내를 제공합니다.

### 🌐 9개 국어 완전 현지화
- **다국어 번역 지원**:
  - 지원하는 모든 9개 언어에 걸쳐 업데이트 관련 버튼, 상태 레이블, 대화 상자 텍스트를 완벽히 추가했습니다.

### 📦 CI/CD 릴리즈 매니페스트 자동 생성 (`latest.json`)
- **배포 자동화 고도화**:
  - `scripts/build_release_packages.sh`를 개선하여 릴리즈 노트 추출, DMG SHA256 체크섬 계산 및 `dist/latest.json` 생성을 자동화했습니다.
  - `.github/workflows/release.yml`을 통해 GitHub Releases에 `latest.json`을 자동으로 업로드합니다.

### 🧪 테스트 및 품질 보증
- **테스트 커버리지 확장**:
  - 자동화 테스트 케이스를 585개로 확장하여 100% 통과율을 달성했습니다.

---

## [v1.6.0] - 2026-09-13

### 🪟 트레이 종료 데드락 해결 및 애플리케이션 종료 라이프사이클 강화
- **안정적인 프로세스 종료 보장**:
  - 앱이 트레이로 최소화된 상태에서 트레이 메뉴 또는 Dock 우클릭 메뉴의 '종료'를 클릭했을 때 앱이 멈추고 종료되지 않던 데드락 결함을 수정했습니다.
  - `ATBCloneApp`에 `_is_exiting` 가드 플래그를 도입하여 종료 절차 진행 중 `_on_window_close`가 창 닫기를 가로채거나 차단하지 않도록 보장했습니다.
  - `exit_application()` 내에서 `set_macos_dock_visible(True)`를 사전에 호출하던 로직을 제거하여 Cocoa 런루프의 재진입 및 활성화 정책 전환으로 인한 종료 방해 현상을 원천 차단했습니다.
  - Dock 종료(`_on_app_exit`) 및 트레이 종료(`exit_application`) 경로를 모두 강화하여 트레이 아이콘을 즉시 해제하고 지연 없이 프로세스를 종료합니다.

---

## [v1.5.1] - 2026-09-13

### 💬 카카오톡(KakaoTalk) 기본 레시피 내장
- **네이티브 다중 인스턴스 지원**:
  - 카카오톡(`com.kakao.KakaoTalkMac`) 기본 내장 레시피를 추가했습니다. Cocoa 하드 클론 전략, 샌드박스 제거 및 독립 데이터 디렉터리(`HOME`/`TMPDIR`) 격리를 지원합니다.
  - 기본 내장 레시피 컬렉션을 34개 이상의 애플리케이션으로 확장했습니다.
  - 검증 범위를 문서에 명시했습니다 (동시 실행 및 시스템 알림 기능 정상 동작 검증).

### 🚀 GitHub Actions arm64 자동 릴리즈 및 패키징 워크플로
- **지속적 배포 및 패키징 자동화**:
  - 버전 태그(`v*`) 푸시 시 자동으로 릴리즈를 생성하는 `.github/workflows/release.yml`을 추가했습니다.
  - CLI 단독 바이너리 압축 파일(`ATBCloneCli-arm64.tar.gz`) 및 GUI 설치 이미지(`ATBClone-*.dmg`)를 자동 빌드하고 SHA256 체크섬을 생성합니다.
  - 통합 릴리즈 패키징 스크립트 `scripts/build_release_packages.sh`를 추가했습니다.
  - `scripts/build_cli.sh` 및 `scripts/build_release_packages.sh`에서 bash `set -u` 빈 배열 확장 오류를 수정했습니다.

### 🛡️ GUI 빌드 스캐폴딩 및 네트워크 탄력성 강화
- **불완전 빌드 자동 복구**:
  - `scripts/build_gui.sh`의 무결성 검사를 강화하여 `Python.framework`, `ATBClone` 실행 파일 및 `Stub`을 철저히 검증합니다.
  - 불안정한 네트워크로 인해 다운로드가 중단된 경우 손상된 스캐폴딩을 자동으로 정리하고 재생성합니다.

### 🔒 보안 정책 문서 추가 (SECURITY.md)
- **책임 있는 취약점 공개 가이드**:
  - `.github/SECURITY.md`를 추가하여 지원 버전 범위, 비공개 취약점 보고 채널, 대응 절차 및 범위를 명확히 규정했습니다.

### 🧪 테스트 및 품질 보증
- **테스트 케이스 확장**:
  - 자동화 테스트 케이스를 561개로 확장하여 100% 통과율을 달성했습니다.
