...
### 📢 導覽側邊欄新增 ATBCmder 推廣卡片與 [AD] 標識
- **生態工具連動與介面升級**：
  - 在導覽側邊欄中間自適應空白區域新增 ATBCmder（`https://aitobox.com/atbcmder`）推廣卡片。
  - 遵循 macOS 原生設計規範，配備半透明圓角卡片背景、醒目的 `[AD]` 廣告標籤、高解析度應用圖示、多語言副標題與外鏈開啟按鈕。
  - 完整接入 9 國語言國際化系統，支援設定中切換語言時卡片標題、描述及操作按鈕文字即時重新整理。
  - 新增 ATBCmder 圖示資源及資源定位函式，具備完善的容錯機制。

### 🛡️ CI/CD 發布說明保存保護與打包流程加固
- **自動化發布穩定性提升**：
  - 修復 `build_cli.sh` 清理 `dist/` 目錄時意外誤刪已擷取的 `dist/release_notes.md` 導致 GitHub Releases 遺失說明檔的問題。
  - 在 `scripts/build_release_packages.sh` 中增加發布說明自動備份與安全還原機制。
  - 在 `.github/workflows/release.yml` 中增加 Git Tag 註釋及提交歷史備援產生機制，確保發布日誌完整無虞。

### 🧪 品質保障與自動化測試
- **測試套件擴充**：
  - 自動化測試案例擴充至 588 項並保持 100% 通過率，全面涵蓋側邊欄卡片排版、外鏈點擊跳轉、多語言動態切換及發布打包腳本。

---

## [v1.7.0] - 2026-09-13

### 🔄 應用程式內自動檢查更新與無訊息安裝子系統
- **端對端自動化升級鏈路**：
  - 新增 `UpdateService` 服務，自動擷取 GitHub 發布資訊清單 `latest.json` 並透過語意化版本比對智慧偵測新版本。
  - 支援架構自適應下載配對（Apple Silicon arm64 與 Intel x86_64 DMG），支援串流分塊下載與即時百分比進度回報。
  - 自動校驗下載 DMG 檔案的 SHA256 總和檢查碼，與發布資訊清單嚴格比對，杜絕損毀或遭竄改安裝檔的風險。
  - 完整繼承全域網路代理設定（`HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY`），保障受限網路環境下的穩定更新。
  - 原生無訊息就地覆蓋安裝：基於隔離暫存掛載點掛載 DMG，替換目標 `ATBClone.app`，自動清理 macOS Gatekeeper 隔離屬性（`xattr -cr`），並安全卸載掛載卷。

### ⚙️ GUI 設定介面更新互動流程
- **視覺化更新狀態推進**：
  - 在 `SettingsView` 設定介面新增「檢查更新」按鈕及即時狀態回饋指示。
  - 提供多階段視覺化狀態切換：檢查中 → 發現新版本（顯示版本號及更新日誌）/ 已是最新 → 下載中（即時進度百分比）→ 安裝中 → 安裝完成 / 重啟提示。
  - 完善的網路逾時、HTTP 異常及總和檢查碼不符等易懂錯誤提示。

### 🌐 9 國語言國際化完整適配
- **更新提示多語言支援**：
  - 在全部 9 種支援語言中完整補齊更新按鈕、狀態標籤、進度提示與對話方塊文案。

### 📦 CI/CD 發布資訊清單自動化產生 (`latest.json`)
- **自動化發布中繼資料**：
  - 升級 `scripts/build_release_packages.sh`，自動擷取發布記錄、計算 DMG SHA256 總和檢查碼並產生 `dist/latest.json`。
  - 最佳化 `.github/workflows/release.yml` 自動同步上傳 `latest.json`，並增強歸檔與重試機制。

### 🧪 品質保障與自動化測試
- **測試套件擴充**：
  - 自動化測試案例擴充至 585 項，全面涵蓋 UpdateService 業務邏輯、UI 狀態流、總和檢查碼配對及 i18n 完整性，保持 100% 通過率。
...
### 🪟 系統匣結束死結消除與結束生命週期加固
- **解決縮小至系統匣結束卡死問題**：
  - 徹底修復應用程式縮小至系統匣時，透過系統匣選單或 Dock 欄右鍵選單點選「結束」導致程式死結、無法正常結束的缺陷。
  - 在 `ATBCloneApp` 中引入 `_is_exiting` 結束保護旗標，確保 `_on_window_close` 視窗關閉攔截在結束流程中立即放行，絕不阻斷處理程序終止。
  - 移除 `exit_application()` 中在結束前過早呼叫 `set_macos_dock_visible(True)` 的邏輯，杜絕還原 Dock 欄啟動策略觸發 Cocoa 事件循環重入與視窗關閉攔截循環。
  - 加固 Dock 欄結束（`_on_app_exit`）與系統匣結束（`exit_application`）雙鏈路，確保系統匣服務乾淨卸載並實現秒級平穩結束。
...
### 💬 內建 KakaoTalk 分身規則
- **開箱即用的原生多開支援**：
  - 新增 KakaoTalk（`com.kakao.KakaoTalkMac`）內建配方，採用 Cocoa 硬複製策略，支援沙箱剝離與獨立資料目錄（`HOME`/`TMPDIR`）隔離。
  - 內建應用程式規則庫擴充至 34+ 款熱門軟體。
  - 補充說明驗證範圍（已驗證雙開同時執行及系統通知功能）。

### 🚀 GitHub Actions arm64 自動發布與打包工作流程
- **持續部署與發布工程化**：
  - 新增 `.github/workflows/release.yml`，在推送版本 Tag（`v*`）時自動觸發發布流程。
  - 自動編譯並打包 CLI 獨立二進位壓縮包（`ATBCloneCli-arm64.tar.gz`）與 GUI 安裝映像檔（`ATBClone-*.dmg`），並附帶 SHA256 總和檢查碼。
  - 新增 `scripts/build_release_packages.sh` 統一發布打包指令碼。
  - 修復 `scripts/build_cli.sh` 與 `scripts/build_release_packages.sh` 中 bash `set -u` 空陣列展開報錯問題。

### 🛡️ GUI 建置鷹架與不穩定網路韌性增強
- **不完整建置自動復原**：
  - 增強 `scripts/build_gui.sh` 鷹架完整性檢查，全面校驗 `Python.framework`、可執行檔 `ATBClone` 與 `Stub`。
  - 在因網路不穩導致資源下載殘缺時，自動清理並重建損壞鷹架，保障建置成功率。

### 🔒 開源安全策略規範 (SECURITY.md)
- **負責任的弱點揭露**：
  - 新增 `.github/SECURITY.md`，明確支援版本範圍、私密弱點提報管道、評估回應時效與受影響元件定義。

### 🧪 品質保障與自動化測試
- **測試套件擴充**：
  - 自動化測試案例擴充至 561 項，涵蓋 KakaoTalk 配方、建置指令碼穩健性與打包邏輯，保持 100% 通過率。
