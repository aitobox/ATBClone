...
@click.option("--proxy-password", default=None, help="Proxy password (stored securely in Keychain)")
...
def clone(
    ...
    if proxy_host:
        try:
            ...
            console.print(f"[bold red]Proxy configuration error:[/bold red] {e}", soft_wrap=True)
            sys.exit(1)
