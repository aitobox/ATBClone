...
class CloneDetailWindow(toga.Window):
    def __init__(self, record: CloneRecord):
        ...
        self.label_source_app = toga.Label(t("win_detail_source_app", source_app=record.source_app), style=Pack(margin=4))
        self.label_source_path = toga.Label(t("win_detail_source_path", path=record.source_path), style=Pack(margin=4))
        self.label_bundle_id = toga.Label(t("win_detail_bundle_id", bundle_id=record.bundle_id), style=Pack(margin=4))
        self.label_new_bundle_id = toga.Label(t("win_detail_new_bundle_id", new_bundle_id=record.new_bundle_id or "—"), style=Pack(margin=4))
        self.label_strategy = toga.Label(t("win_detail_strategy", strategy=strat_badge), style=Pack(margin=4))
        self.label_dest_path = toga.Label(t("win_detail_dest_path", dest_path=record.dest_path), style=Pack(margin=4))
        self.label_data_dir = toga.Label(t("win_detail_data_dir", data_dir=record.data_dir), style=Pack(margin=4))
        self.label_created_at = toga.Label(t("win_detail_created_at", created_at=record.created_at), style=Pack(margin=4))
        self.label_proxy = toga.Label(t("win_detail_proxy", proxy=proxy_str), style=Pack(margin=4))
