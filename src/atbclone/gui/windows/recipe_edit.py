...
class RecipeEditWindow(toga.Window):
    ...
    def _build_content(self) -> toga.Box:
        ...
        row1.add(toga.Label("Bundle ID:", style=Pack(width=120)))
        row1.add(self.input_bundle_id)
        form_box.add(row1)

        # App Name
        row2 = toga.Box(style=Pack(direction=ROW, margin=5))
        row2.add(toga.Label("App Name:", style=Pack(width=120)))
        row2.add(self.input_app_name)
        form_box.add(row2)

        # Strategy
        row3 = toga.Box(style=Pack(direction=ROW, margin=5))
        row3.add(toga.Label("Strategy:", style=Pack(width=120)))
        row3.add(self.select_strategy)
        form_box.add(row3)

        # Strip Sandbox
        row4 = toga.Box(style=Pack(direction=ROW, margin=5))
        row4.add(toga.Label("Sandbox:", style=Pack(width=120)))
        row4.add(self.switch_strip_sandbox)
        form_box.add(row4)

        # Proxy section
        proxy_heading = toga.Label("Proxy Settings", style=Pack(margin_top=15, font_weight="bold"))
        ...
        row_proxy_detail.add(toga.Label("Type/Host/Port:", style=Pack(width=120)))
    ...
    async def on_save_press(self, widget: toga.Button):
        ...
        if not recipe.bundle_id or not recipe.app_name:
            await self.error_dialog("Validation Error", "Bundle ID and App Name cannot be empty.")
            return
